...
def DNN_Test(cleanedString):
	...
	testX = numpy.array(testX).reshape([1, 400001])

	print("DNN_Test: Loading model.")
	model = load_model('Keras_Train_ANN.model')
	predictions = model.predict(testX)
	probabilitiesWithHashtags = [('#'+bagOfHashtags[i], predictions[0][i]) for i in range(len(predictions[0]))]

	print("DNN_Test: Done.")
	DNN_Return = sorted(probabilitiesWithHashtags, reverse = True, key = lambda x: x[1])[:20]

string = " ".join(sys.argv[1:])
cleanedString, _ = CleanStringAndExtractHashtags(string)
DNN_Test(cleanedString)
